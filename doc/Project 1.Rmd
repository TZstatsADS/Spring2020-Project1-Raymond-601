...
knitr::include_graphics("../figs/Metal.jpg")
```
...
I am using the cleaned data of $lyrics.csv$ using Text_Processing_a_little_change in the $doc$ file
...
Then I select the Metal songs from 1985 to 1995, which is the period that my Dad's generation listening to Music, and Metal songs from 2010 to 2016, which is the period that my generation listening to Music. 
...
From these two wordcloud, we can see that Metal music from our dad's generation tends to talk more about Love, Life, Death, Sky, Night, whereas those from our generation mainly focus on Time, Life and World. Maybe we youth are just becoming more sensitive to how big our world is and how fast the time passes by.
...
We can also tell that the diversity of the emotion changed a little bit. It verify the 
...
ggplot(dt_Q3, aes(x = period, y = Freq))+
...
### Additional topics: Is the word diversity really getting lower?
